...
const Home = () => {
  ...
  Catogeries = Catogeries.filter(
    (item, index) => Catogeries.indexOf(item) === index
  );
    ...
    return (
      <>
        <nav className="categories">
          ...
          {Catogeries.map((el) => (
            <button key={el} onClick={() => setSearch(el)} className="category">
              {el}
            </button>
          ))}
  ...
  return (
    <>
      <div className="body">{data.length > 0 ? loaded() : loader()}</div>
    </>
  );
};
